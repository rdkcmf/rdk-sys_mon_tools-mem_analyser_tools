...
# this can be used to check the speed of memeory in case of random numbers writes. 
# keep in mind rand() takes a lot of cpu cycles and speed will be way lower tha cont writes
# you should also run const check to get full picture
...
./memstress $RUN_TILL_DEATH $WAIT_AFTER_ALLOCATION $NUM_OF_THREADS $TOTAL_ALLOC_SIZE $ALLOC_BLOCK_SIZE $SLEEP_BW_ALLOCS $KEEP_MEM_HOT -- > $MEMSTRESS_LOG_FILE
pkill vmstat
